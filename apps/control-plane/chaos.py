...
class ChaosReport:
    ...
    def print(self):
        ...
        print(f"  Time-to-detect: {self.time_to_detect:.1f}s" if self.time_to_detect else "  Time-to-detect: —")
        print(f"  Time-to-fix:    {self.time_to_fix:.1f}s"    if self.time_to_fix    else "  Time-to-fix:    —")
        print(f"  Memory recall:  {'✅ fired' if self.recall_fired else '❌ not fired'}")
  ...
  async fetch(request) {{
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            print(f"  ✅ Chaos injected — {action.target_region} now returns +{delay}ms")
            return True
        else:
        ...
        while time.monotonic() < deadline:
            try:
                ...
                if detected_at is None and rtt > 200:
                    ...
                    print(f"  🔍 Breach detectable at RTT={rtt:.0f}ms (t+{detected_at - injected_at:.1f}s)")

                if detected_at and fixed_at is None and rtt < 150:
                    fixed_at = time.monotonic()
                    print(f"  ✅ Recovery detected at RTT={rtt:.0f}ms (t+{fixed_at - injected_at:.1f}s)")
                    break
...
async def run_chaos_cycle(demo_mode: bool = True) -> ChaosReport:
    print(f"\n{'─'*60}")
    ...
    # Generate hypothesis
    print(f"  🤖 Generating chaos hypothesis...")
...
async def run_scheduler(demo_mode: bool = True):
    ...
    print(f"{'='*60}")
    print(f"  NebulaStream Chaos Agent")
...
async def run_once_and_report(demo_mode: bool = True, report_path: str | None = None):
    ...
    if report and report_path:
        ...
        with open(md, "w") as f:
            ...
            f.write(f"- SLA (<{SLA_THRESHOLD_SEC}s): {'PASS' if report.sla_passed else 'FAIL'}\n")
    return report
...
if __name__ == "__main__":
    ...
    parser.add_argument("--once", action="store_true", help="Run a single chaos cycle and exit")
    parser.add_argument("--prod", action="store_true", help="Production schedule (30min interval)")
    parser.add_argument("--report", default="reports/chaos-report.json", help="Where to write report JSON")
    args = parser.parse_args()
    demo = not args.prod
    try:
        if args.once:
            report = asyncio.run(run_once_and_report(demo_mode=demo, report_path=args.report))
            import sys
