...
class Settings:
    workers: dict = field(
        default_factory=lambda: {
            "us-east": _env(
                "WORKER_US_EAST_URL", "https://nebula-us.nebulastream.workers.dev"
            ),
            "eu-west": _env(
                "WORKER_EU_WEST_URL", "https://nebula-eu.nebulastream.workers.dev"
            ),
            "ap-south": _env(
                "WORKER_AP_SOUTH_URL", "https://nebula-apac.nebulastream.workers.dev"
            ),
        }
    )

    poll_interval_sec: int = field(
        default_factory=lambda: _env_int("POLL_INTERVAL_SEC", 5)
    )
    breach_threshold_ms: float = field(
        default_factory=lambda: _env_float("BREACH_THRESHOLD_MS", 200.0)
    )
    ...
    ollama_embed_url: str = field(
        default_factory=lambda: _env(
            "OLLAMA_EMBEDDINGS_URL", "http://host.docker.internal:11434/api/embeddings"
        )
    )
    ollama_model: str = field(default_factory=lambda: _env("OLLAMA_MODEL", "llama3.2"))

    milvus_host: str = field(
        default_factory=lambda: _env("MILVUS_HOST", "milvus-standalone")
    )
    ...
    veto_enabled: bool = field(
        default_factory=lambda: _env("VETO_ENABLED", "true").lower() == "true"
    )
    veto_threshold: float = field(
        default_factory=lambda: _env_float("VETO_THRESHOLD", 0.4)
    )
