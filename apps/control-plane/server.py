...
from memory import recall_cases, store_incident, update_outcome, Incident
...
app = FastAPI(title="NebulaStream Control Plane", version="0.2.0")
...
class State:
    ...
    def p95(self, region):
        if not self.readings[region]: return 0.0
...
async def chaos_inject(region: str = "eu-west", delay: int = 400):
    if region not in WORKERS:
        event = {"type":"chaos","action":"inject","region":region,
    ...
    event = {"type":"chaos","action":"inject","region":region,
             "delay_ms":delay,"success": True,"mode":"in-memory"}
    state.chaos_log.append(event)
    await broadcast(event)
    return event
...
async def chaos_clear(region: str = "eu-west"):
    if region not in WORKERS:
        event = {"type":"chaos","action":"clear","region":region,
                 "success": False, "error": f"unknown region {region}"}
        state.chaos_log.append(event)
        await broadcast(event)
        return event
...
Respond ONLY with JSON: {{"weights":{{"us-east":<int>,"eu-west":<int>,"ap-south":<int>}},"reasoning":"<one sentence>"}}"""
    try:
        async with httpx.AsyncClient(timeout=30.0) as client:
            r = await client.post(OLLAMA_URL, json={"model":OLLAMA_MODEL,"prompt":prompt,"stream":False})
        ...
        log.warning("llm_proposal_failed", error_type=type(ex).__name__, error=str(ex)[:200])
        w2, why = rule_based_proposal(latencies, weights)
        return w2, why, True


async def monitor_recovery(region: str, weights_before: dict, weights_after: dict,
                           fix_applied: str, breach_latency: float):
    ...
    while time.monotonic() - start < timeout:
        ...
        if p95 and p95 < BREACH_THRESHOLD_MS:
            ...
            m.recovery_seconds.labels(region=region, outcome="recovered").observe(elapsed)
            await update_outcome(
                region=region, fix_applied=fix_applied, latency_ms=breach_latency,
                weights_before=weights_before, weights_after=weights_after,
                recovery_seconds=elapsed, fix_worked=True,
            )
    ...
    await update_outcome(
        region=region, fix_applied=fix_applied, latency_ms=breach_latency,
        weights_before=weights_before, weights_after=weights_after,
        recovery_seconds=float(timeout), fix_worked=False,
    )
    m.incidents_stored_total.inc()
    ...
    async with httpx.AsyncClient() as client:
        while True:
            ...
            if breaching and can_shift:
                ...
                m.llm_calls_total.labels(outcome="error" if used_fallback else "success").inc()
                ...
                if settings.veto_enabled and not used_fallback:
                    ...
                    if verdict.cases_considered < settings.veto_min_cases:
                        ...
                        safe, safe_reason = rule_based_proposal(latencies_p95, weights_before)
                        log.warning(
                            "mvlc_veto",
                        ...
                        proposed   = safe
                        reasoning  = f"[VETOED by memory] LLM said: {reasoning}. " + safe_reason
                        used_fallback = True
                    else:
                ...
                # ── 3. Apply ────────────────────────────────────────────────
                m.shifts_total.labels(outcome="fallback" if used_fallback else "llm").inc()
                ...
                asyncio.create_task(monitor_recovery(
                    region=primary_region,
